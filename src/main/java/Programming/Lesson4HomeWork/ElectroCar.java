...
public class ElectroCar extends Car{
    ...
    public ElectroCar(String company, String model, int year) throws Exception {
        ...
        cost = (int) (Math.random()*1_000_000 + 1_000_000);
        numberOfHorsepower = (int) (Math.random() * 200) + 100;
    }
    ...
    public void fillLevel(int filles) {
        ...
        System.out.println("Сейчас столько энергии " + levelResourseToDrive + "%");
    }
    ...
    public void drive() {
        if (levelResourseToDrive == 0){
            System.out.println("У машины " + company + " " + model  + " Нет электричества");
        }
        else{
            System.out.println(company + " " + model + " поехала");
            int x = (int)(Math.random()*5 + 1);
            levelResourseToDrive = Math.max(levelResourseToDrive - x, 0);
            System.out.println(getLevelResourseToDrive() + "% Столько осталось энергии");
        }
    }
